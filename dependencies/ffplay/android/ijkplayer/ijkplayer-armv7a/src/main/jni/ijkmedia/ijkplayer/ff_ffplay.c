...
static void stream_close(FFPlayer *ffp)
{
    ...
    VideoState *is = ffp->is;
    printf("stream_close avformat_preclose_input");
avformat_preclose_input(&is->ic);
  ...
  //  SDL_DetachThread(is->read_tid);
    ...
    av_log(NULL, AV_LOG_ERROR, "wdm=================================call avformat_close_input===============\n");
...
static int read_thread(void *arg)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "wdm======================g_count========%d\n",g_count);
    if(g_count == 0){
		for(;;){
			         SDL_Delay(2000);
				   av_log(NULL, AV_LOG_ERROR, "wdm======================test for hangup.\n");
          			  continue;
		}
    }

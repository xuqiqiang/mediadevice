...
FF_MODULE_DIRS="compat libavcodec libavfilter libavformat libavutil libswresample libswscale librtmp"
...
if [ "$FF_ARCH" = "armv7a" ]; then
    ...
    FF_BUILD_NAME_RTMPDUMP=rtmpdump-armv7a
    ...
    FF_BUILD_NAME_RTMPDUMP=rtmpdump-armv5
    FF_SOURCE=$FF_BUILD_ROOT/$FF_BUILD_NAME
    ...
    FF_BUILD_NAME_RTMPDUMP=rtmpdump-ax86
    FF_SOURCE=$FF_BUILD_ROOT/$FF_BUILD_NAME
    ...
    FF_BUILD_NAME_RTMPDUMP=rtmpdump-x86_64
    FF_SOURCE=$FF_BUILD_ROOT/$FF_BUILD_NAME
...
FF_DEP_POLARSSL_OBJS=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_POLARSSL/output/objs
...
FF_DEP_RTMPDUMP_INC=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_RTMPDUMP/output/usr/local/include
FF_DEP_RTMPDUMP_LIB=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_RTMPDUMP/output/usr/local/lib
FF_DEP_RTMPDUMP_OBJS=$FF_BUILD_ROOT/build/$FF_BUILD_NAME_RTMPDUMP/output/usr/local/objs
FF_SOURCE_RTMP_OBJS=$FF_SOURCE/librtmp/arm
...
if [ -f "${FF_DEP_POLARSSL_LIB}/libpolarssl.a" ]; then
    ...
    cp -fp $FF_DEP_POLARSSL_OBJS/*.o $FF_SOURCE_RTMP_OBJS
else
...
if [ -f "${FF_DEP_RTMPDUMP_LIB}/librtmp.a" ]; then
    ...
    cp -fp $FF_DEP_RTMPDUMP_OBJS/*.o $FF_SOURCE_RTMP_OBJS
else
    echo "You should compile rtmpdump before";
    exit 1
fi
...
FF_CFG_FLAGS="$FF_CFG_FLAGS --extra-libs=\"-lrtmp\""
FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-protocol=file --enable-protocol=rtmp"
FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-muxer=mov --enable-muxer=ipod --enable-muxer=psp --enable-muxer=mp4 --enable-muxer=flv --enable-muxer=avi"
FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-demuxer=h264 --enable-demuxer=avi --enable-demuxer=mpc --enable-demuxer=mov"
FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-decoder=aac --enable-decoder=aac_latm --enable-decoder=h264 --enable-decoder=mpeg4"
FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-parser=aac --enable-parser=ac3 --enable-parser=h264"

FF_EXTRA_CFLAGS="$FF_EXTRA_CFLAGS -I$FF_DEP_POLARSSL_INC -I$FF_DEP_OPENSSL_INC -I$FF_DEP_RTMPDUMP_INC"
FF_EXTRA_LDFLAGS="-L$FF_DEP_POLARSSL_LIB -L$FF_DEP_OPENSSL_LIB -L$FF_DEP_RTMPDUMP_LIB $FF_EXTRA_LDFLAGS"

cd $FF_SOURCE
find -name 'configure' | xargs perl -pi -e 's|require_pkg_config librtmp librtmp/rtmp.h RTMP_Socket|require librtmp librtmp/rtmp.h RTMP_Socket -lrtmp -lpolarssl -lz|g'
#--------------------
echo ""
echo "--------------------"
echo "[*] configurate ffmpeg"
echo "--------------------"
FF_DEP_LIBS="$FF_DEP_LIBS -llog"
if [ -f "./config.h" ]; then

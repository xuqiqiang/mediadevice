...
printf("tcp_write 2, size:%d, buf:%s", size, buf);
    ret = send(s->fd, buf, size, MSG_NOSIGNAL);
printf("tdp_write 3, ret:%d, ff_neterrno():%d", ret, ff_neterrno());
printf("tdp_write 4, AVERROR(EAGAIN):%d, VERROR(EINTR):%d", AVERROR(EAGAIN), AVERROR(EINTR));
    return ret < 0 ? ff_neterrno() : ret;
}
